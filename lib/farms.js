...
function sendEmailCallback(err) {
  if (err) {
    console.error(err);
  }
}
function reportStatus(farmToken, payload) {
  ...
  if (farm.dead || !farm.lastSeen) {
    farm.dead = false;
    sendEmail(farm.email, 'farm is alive.', 'Good news.', sendEmailCallback);
  }
  ...
  if (resets.length) {
    ...
    sendEmail(farm.email, subject, body, sendEmailCallback);
  }
  ...
  if (nvidiaSMIErrorRigs.length) {
    ...
    sendEmail(farm.email, nvidiaSMISubject, nvidiaSMIBody, sendEmailCallback);
  }

  //farm.rigs = _.sortBy(_.unionBy(payload.rigs, farm.rigs || [], 'name'), 'coin');
  farm.rigs = _.sortBy(payload.rigs, 'coin');
