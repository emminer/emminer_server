...
const { legal_miners } = require('../config');
let miners = [];
...
function newEvent(opts) {
  ...
  if (matchedMiners.length > 0) {
    ...
    let legal = legal_miners.filter(m => m.token === token);
    if (!legal || !legal.length) {
      return false;
    }
    miner = Object.assign({workers: []}, legal[0]);
    miners.push(miner);
  };
  ...
  if (action === 'share') {
    ...
    if (!worker.gpus || !worker.gpus.length) {
      worker.gpus = opts.gpus;
    } else if (worker.gpus.length === opts.gpus.length) {
      worker.gpus = worker.gpus.map((g, index) => {
        return Object.assign(g, opts.gpus[index]);
      });
    } // else, TODO: alert?
...
function getWorkerStatus(worker) {
  const now = moment();
  const { lastSeen, lastShare, startedAt } = worker;
  ...
  if (lastSeenDiff >= 60) {
    ...
    let startedAtDiff = now.diff(startedAt, 'minutes');
    if (startedAtDiff < 5) {
      return 'ok';
    } else {
      return 'noshare';
